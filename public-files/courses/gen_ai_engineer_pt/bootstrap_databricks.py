...
# MAGIC %md-sandbox
# MAGIC <div style="background:linear-gradient(135deg,#6B3FA0 0%,#0077B6 100%);color:#fff;padding:44px 38px;border-radius:14px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;box-shadow:0 6px 24px rgba(107,63,160,.25)">
# MAGIC   <div style="font-size:.85em;text-transform:uppercase;letter-spacing:.12em;opacity:.85;margin-bottom:8px">Gen AI Engineer Associate · bootstrap · ~2 min</div>
# MAGIC   <h1 style="font-size:2.4em;margin:0 0 14px;line-height:1.15;font-weight:700">Baixar curso completo (PT-BR)</h1>
# MAGIC   <p style="margin:0;font-size:1.1em;opacity:.95;line-height:1.55">Este notebook é a porta de entrada. Cole sua <strong>chave de acesso</strong> e rode todas as células — os <strong>16 notebooks</strong> do curso em português vão aparecer no seu workspace.</p>
# MAGIC </div>

# COMMAND ----------

# MAGIC %md-sandbox
# MAGIC <div style="background:#fff;border:2px solid #6B3FA0;border-radius:10px;padding:24px 28px;font-family:-apple-system,sans-serif;margin:18px 0">
# MAGIC   <h3 style="margin:0 0 12px;color:#6B3FA0;font-size:1.2em">📋 O que este notebook faz</h3>
...
# MAGIC     <li>Materializa os 16 notebooks em <code>gen_ai_engineer/pt_br/</code></li>
# MAGIC   </ol>
# MAGIC </div>
...
ACCESS_KEY = "GEN-AI-ENGINEER-PT-2026"  # ⚠️ modo teste — substituir por "" antes do lançamento
...
PAYLOAD_URLS = [
    "https://raw.githubusercontent.com/AprenderDados/public-files/main/public-files/courses/gen_ai_engineer_pt/payload.private.json",
]
...
target_root = "/Workspace" + os.path.dirname(notebook_path) + "/gen_ai_engineer"
...
try:
    ...
    _key_path = f"/Workspace/Users/{_current_user}/.gen_ai_engineer_pt_key"
    with open(_key_path, "w") as _kf:
        _kf.write(ACCESS_KEY.strip())
    print(f"🔑 Chave persistida em {_key_path}")
    print("   O notebook do assistente AI (Athena) vai pegar a chave automaticamente.")
...
# MAGIC %md-sandbox
# MAGIC <div style="background:linear-gradient(135deg,#6B3FA0,#0077B6);color:#fff;padding:28px 32px;border-radius:14px;font-family:-apple-system,sans-serif;margin:24px 0;text-align:center">
...
# MAGIC   <p style="margin:0;font-size:1.05em;opacity:.95;line-height:1.55">Abra a pasta <strong>gen_ai_engineer/</strong> no seu workspace.<br>Comece pelo primeiro notebook em <code>pt_br/</code> e siga a ordem dos números.</p>
# MAGIC </div>
